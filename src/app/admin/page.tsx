...
export default function AdminHome() {
  ...
  return (
    <RequireAdmin>
      <AdminLayout>
        <h1 className="text-xl font-bold text-gray-800  text-gray-800 mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <Link href="/admin/eventos" className="bg-white rounded-xl shadow p-4 flex items-center gap-4 border hover:shadow-md">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-gray-800  text-gray-800">{proximosEventos.length}</p>
            </div>
          </Link>
          ...
          <Link href="/admin/pessoas" className="bg-white rounded-xl shadow p-4 flex items-center gap-4 border hover:shadow-md">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-gray-800  text-gray-800">{totalPessoas}</p>
            </div>
          </Link>
          ...
          <Link href="/admin/empresas" className="bg-white rounded-xl shadow p-4 flex items-center gap-4 border hover:shadow-md">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-gray-800  text-gray-800">{totalEmpresas}</p>
            </div>
          </Link>
        </div>
        ...
        {/* Próximos eventos */}
        <div className="mb-6">
          <h2 className="text-lg font-semibold text-gray-800  text-gray-800 mb-2">Próximos eventos</h2>
          <ul className="space-y-2">
            {proximosEventos.map(evento => (
              <li key={evento.id} className="bg-white p-4 rounded-xl shadow border">
                <p className="font-bold text-gray-800  text-gray-800">{evento.nome}</p>
